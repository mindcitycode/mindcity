...
export const SpriteRenderer = async (canvas, imagesUrls) => {
    ...
    const textureFromImageUrl = imageUrl => textures[imagesUrls.indexOf(imageUrl)]

    const tiles = []
    const makeTile = (imageUrl, frameX, frameY, frameW, frameH) => {
        const num = tiles.length
        const texture = textureFromImageUrl(imageUrl)
        const tile = new Tile(texture, frameX, frameY, frameW, frameH)
        tiles.push(tile)
        return num
    }
